...
let gameWidth = (snakeGame.style.width =
  Math.floor(snakeGame.offsetHeight / 100) * 100);
let gameHeight = (snakeGame.style.height =
  Math.floor(snakeGame.offsetWidth / 100) * 100);
...
function movingSnake(direction) {
  ...
  for (let i = 0; i < tailElements.length; i++) {
    ...
    if (
      head.style.left == tailElements[i].style.left &&
      head.style.top == tailElements[i].style.top
    ) {
      removeSnake();
    }
  }
